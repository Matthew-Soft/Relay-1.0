...
module.exports = class MusicTriviaCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      group: 'music',
      description: 'Membuat Kuis Musik bersama temanmu!',
      guildOnly: true,
      ...
      args: [
        {
          key: 'numberOfSongs',
          prompt: 'Berapa lagu yang ingin di inginkan?',
          type: 'integer',
          min: 1,
          //default: 5,
          max: 15
  ...
  async run(message, { numberOfSongs }) {
    ...
    if (message.guild.musicData.isPlaying === true)
      return message.channel.send(':x: Ada Kuis atau lagu yang sedamg diputar!');
    ...
    const infoEmbed = new MessageEmbed()
      .setColor('#ff7373')
      .setTitle(':notes: Ayo Kita mulai Kuis nya!')
      .setDescription(
        `:notes: Siap-siap! Ada ${numberOfSongs} Lagu, Kamu punya waktu 30 detik untuk menebak Nama lagu/Penyanyi nya. Good luck!
        Kamu dapat mengakhiri kuis kapan saja dengan perintah ${prefix}stop-kuis!`
      );
    ...
    queue[0].voiceChannel.join().then(function(connection) {
      const dispatcher = connection
        ...
        .on('error', async function(e) {
          message.say(':x: Gagal memutar lagu itu!');
          console.log(e);
          if (queue.length > 1) {
            queue.shift();
        ...
        .on('start', function() {
          ...
          collector.on('end', function() {
            ...
            */
            ...
            const embed = new MessageEmbed()
              .setColor('#ff7373')
              .setTitle(`:musical_note: Lagunya adalah:  ${song}`)
              .setDescription(
                classThis.getLeaderBoard(Array.from(sortedScoreMap.entries()))
              );
